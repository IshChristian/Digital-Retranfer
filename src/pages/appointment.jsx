...
import { Search, Plus, Edit, Trash2, Eye, X, Save, Calendar, Clock, ChevronUp, ChevronDown, ChevronsUpDown } from "lucide-react";
...
const AppointmentPage = () => {
    ...
    const filteredBorns = useMemo(() => {
        ...
        return borns.filter(born => 
            ...
            (born.fatherNationalId && born.fatherNationalId.toLowerCase().includes(bornSearchTerm.toLowerCase()))
        );
    ...
    const filteredAndSortedAppointments = useMemo(() => {
        ...
        if (searchTerm.trim()) {
            filtered = filtered.filter(appointment => {
                ...
                const matchesBorn = appointment.born && (
                    ...
                    (appointment.born.fatherNationalId && appointment.born.fatherNationalId.toLowerCase().includes(searchTerm.toLowerCase()))
                );
...
{currentAppointment ? (isEditMode ? 'Edit Appointment' : 'Appointment Details') : 'New Appointment'}
</h2>
...
<label className="block text-sm font-medium text-gray-700 mb-1">Associated Born Record</label>
...
{born.motherName} (ID: {born.motherNationalId}){born.fatherName && ` & ${born.fatherName} (ID: ${born.fatherNationalId})`}
</option>
))}
</select>
</div>
...
{isPediatrician && currentAppointment.status === "Completed" && !showFeedbackForm && (
<button
onClick={() => {
setFeedbackForm({
...
{currentAppointment.feedback?.length ? 'Add More Feedback' : 'Add Feedback'}
</button>
)}
</div>
...
onChange={(e) => setFeedbackForm({...feedbackForm, nextAppointmentDate: e.target.value})}
/>
</div>
...
setFeedbackForm({
...fb,
babyId: fb.babyId || currentAppointment.born?.babies?.[0]?.id || '',
appointmentId: currentAppointment.id
});
setShowFeedbackForm(true);
}}
